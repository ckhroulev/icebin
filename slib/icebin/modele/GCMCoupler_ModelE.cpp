...
/** If a file is a symlink, reads it.  Otherwise, just returns the file.
TODO: Put in ibmisc */
std::string const read_symlink_or_file(std::string const &path0)
{
    boost::system::error_code ec;
    boost::filesystem::path path1(
        boost::filesystem::read_symlink(path0, ec));

    if (ec.value() == 0) {
        return path1.string();    // It was a symlink
    } else {
        return path0;    // It was not a symlink
    }
}
...
void gcmce_io_rsf(GCMCoupler_ModelE *self,
    char *modele_fname_c, int modele_fname_n, char rw)
{
    ...
    // modele_fname is filename of main ModelE restart file, without
    // .nc extension.  However, we need to read subsidiary restart files.
    // For example, if modele_root = fort.1, restart files are:
    //     fort.1.nc           (Main ModelE restart file)
    //     fort.1-icebin.nc    (IceBin restart file),
    //     fort.1-greenland.nc (Restart file generated by ice model)
    //
    // IF WRITING:
    //    We must generate subsidiary rsf names and write to them.
    //
    // IF READING:
    //    The main ModelE rsf (eg fort.1.nc) is probably a symlink,
    //    and there are no symlinks to the subsidiary restart files.
    ...
    // Save the state of each ice model.
